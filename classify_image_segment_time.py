...
class NamedPipe:
    ...
    def read_and_write_to_queue(self, pid, model_name, server_write_path, client_write_path):
        ...
        prev_read = 0
        until_server_read_message_time = 0
        i = 0
        sum = 0
        while True:
            ...
            # queue_start = time.perf_counter()
                ...
                # print('request time = {}  , now = {}'.format(request_time, time.perf_counter()))
                # until_server_read_message_time = (time.perf_counter() - request_time)
                # from_while = (time.perf_counter() - while_start) * 1000
                # print('------------------------------------------------------')
                # print('until server read mesaage = {}'.format(until_server_read_message_time * 1000))
                # until_server_read_message_time += until_server_read_message_time
                
                # sum += until_server_read_message_time
                # print(until_server_read_message_time * 1000)
                # sum = 0
                # if i >= 1:
                #     i = 0
                #     avg = sum / 10
                ...
                # if until_server_read_message_time * 1000 > 100:
                #     exit()
                # print('from while = {}'.format(from_while))
                # if until_server_read_message_time*1000 > 100 :
                #     exit()
                ...
                ####################################################3
                # preempt_lock_start = datetime.now()
                ...
                # preempt_lock_release = datetime.now()
                ########################################################

                #####################################################
                # rekquest_lock_start = datetime.now()
                # segment_num = int(segment_num)

                # request_list_lock_acquire_start = time.perf_counter()
                # lock.acquire()
                # NamedPipe.request_list.append( (pid, image_path, request_time, period, segment_num))

                # lock.release()
                # request_list_lock_acquire_time = time.perf_counter() - request_list_lock_acquire_start
                # request_lock_release = datetime.now()
                # print('acquring preempt lock = {} '.format((preempt_lock_release - preempt_lock_start) * 1000))

                # print('acquirng request lock = {}'.format((request_lock_release - request_lock_start) * 1000))
                # #####################################################


                # print('until server_read_message to put request = {}'.format( (request_lock_release -preempt_lock_start) * 1000 ))
            ...
            if execute_segment_flag:
                ...
                for i in range(segment_num):
                    ...
                    # executing last segment
                        ...
                        # print('while end  = {}'.format(while_end))
