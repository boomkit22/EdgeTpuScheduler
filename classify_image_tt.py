...
class NamedPipe:
    ...
    def make_client_pipe(self):
        ...
        while True:
            ...
            if readmsg:
                ...
                to_client = os.open(ServerWritePath, os.O_WRONLY)
                from_client = os.open(ClientWritePath, os.O_RDONLY)
                pid = int(pid)
                process_dict[pid] = (modelName, to_client)
                while True:
                    ...
                    if msg:
                        until_arive = (time.perf_counter() - request_time) * 1000
                        print(until_arive)
                        # lock.acquire()
                        # request_list.append((pid, image_path, request_time, period))
                        # lock.release()
    ...
    def read_and_write_to_queue(self, pid, model_name, server_write_path, client_write_path):
        ...
        pid = int(pid)
        process_dict[pid] = (model_name, to_client)
        while True:
            msg = os.read(from_client, 100).decode()
            try:
                pid, image_path, request_time, period = msg.split(' ')
...
class Scheduler:
    ...
    def schedule(self):
        ...
        #     x = 3
        #     x = 3
            # if ( time.perf_counter() - start )  > ( check_period / 1000 ) * iter:
            #     iter = iter + 1
            #     print(iter)
                # lock.acquire()
                ...
                #     # assigned_interpreter = self.interpreter.model_interpreter_dict[modelName]
    ...
    def run(self):
        # self.named_pipe = NamedPipe()
        # self.named_pipe.run()
        # self.interpreter = Interpreter()
        # self.interpreter.initialize_dict()
