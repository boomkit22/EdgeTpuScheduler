    ...
    def read_and_write_to_queue(self, pid, model_name, server_write_path, client_write_path):
        ...
        while True:
            ...
            if priority > NamedPipe.current_task["priority"] and NamedPipe.current_task["priority"] != 0 :
                # print('priority : {} , NamedPipe.current_task["priority"]: {}'.format(priority,NamedPipe.current_task["priority"]))
...
## segment를 실행하고 preempt flag를 확인한다..
...
class Scheduler:
    ...
    def schedule(self):
         while True:
            ...
            if len(self.intermediate_output) != 0:
                # print('executing preempted segment')
                # print(len(self.intermediate_output))
                # print(NamedPipe.preempt_flag)
                # print(self.intermediate_output)
                # 있으면 priority 제일 높은 segment부터 실행
                self.intermediate_output.sort(key = lambda x : x["priority"]/1)
                ...
                for i in range(next_index,segment_num):
                    ...
                    if i == segment_num - 1:
                        # print('i == segment_num - 1, i =={}'.format(i))
                        assigned_interpreter = interpreter.model_interpreter_dict[modelName][i]
                        common.set_input(assigned_interpreter, segment_intermediate_output) 
                        assigned_interpreter.invoke()
                        tensor_index = assigned_interpreter.get_output_details()[0]['index']
                    ...
                    # executing middle segment
                        ...
                        assigned_interpreter.invoke()
                        tensor_index = assigned_interpreter.get_output_details()[0]['index']
                        segment_intermediate_output = assigned_interpreter.get_tensor(tensor_index)
            ...
            elif len(NamedPipe.request_list) != 0:
